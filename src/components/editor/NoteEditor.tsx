  ...
  useEffect(() => {
    ...
    const handle = async (event: Event) => {
      try {
        ...
        if (action === "find") {
          noteFind.openFind();
        } else if (action === "ask-mark") {
          useUi.getState().toggleAiSidebar();
        } else if (action === "insert-ai-text") {
          const detail = (event as CustomEvent<{ text?: string; replace?: boolean }>).detail;
          if (detail?.text) {
  ...
  useEffect(() => {
    if (active) {
      const syncContext = () => {
        ...
        const content = editor ? editor.getMarkdown() : rawText;
        useUi.getState().setActiveNoteContext({
          rel,
          title: noteTitle(rel),
          content,
