...
export function restoreSession(root: string) {
  ...
  // Drop tabs whose notes were deleted since last session (preserve special tabs like __todos__, __canvas:id__).
  const tabs = (data.tabs ?? []).filter((rel) => rel.startsWith("__") || existing.has(rel));
